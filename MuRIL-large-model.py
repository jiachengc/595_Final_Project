...
random.seed(100)
folder_name = 'MURIL_large'
total_folder_name = folder_name
SAMPLE = False
logger = logging.getLogger(__name__)

folder_name = 'pretraining_MUR_122'
...
def prepare_datasets(fold, seed, sample=False):
    ...
    tokenized_chaii_train_strid2 = chaii_train_ds.map(prepare_train_features,
                                                      fn_kwargs={'max_length': 448, 'doc_stride': 192}, batched=True,
                                                      remove_columns=chaii_train_ds.column_names, batch_size=32)
    ...
    tokenized_hindi_strid1 = hindi_ds.map(prepare_train_features, fn_kwargs={'max_length': 384, 'doc_stride': 192},
                                          batched=True, remove_columns=hindi_ds.column_names, batch_size=32)
    tokenized_hindi_strid2 = hindi_ds.map(prepare_train_features, fn_kwargs={'max_length': 448, 'doc_stride': 224},
                                          batched=True, remove_columns=hindi_ds.column_names, batch_size=32)

    squad = load_dataset("squad")
    tokenized_squad = squad['train'].map(prepare_train_features, fn_kwargs={'max_length': 384, 'doc_stride': 192},
    ...
    tokenized_tydi_bete_all_strid2 = tydi_bete_all_ds.map(prepare_train_features,
                                                          fn_kwargs={'max_length': 448, 'doc_stride': 224},
                                                          batched=True, remove_columns=tydi_bete_all_ds.column_names,
                                                          batch_size=32)
    ...
    tokenized_nq_strid = nq_ds.map(prepare_train_features, fn_kwargs={'max_length': 448, 'doc_stride': 224},
...
for fold in [0, 2]:
    ...
    trainer.train()
    trainer.save_model(f"{final_folder_name}/final_fold{fold}")
